...
FILEPATH_R = WEIGHT_PATH + '\dim1\R\weight.{epoch:02d}-{val_loss:.10f}.weights.h5'
FILEPATH_G = WEIGHT_PATH + '\dim1\G\weight.{epoch:02d}-{val_loss:.10f}.weights.h5'
FILEPATH_B = WEIGHT_PATH + '\dim1\B\weight.{epoch:02d}-{val_loss:.10f}.weights.h5'


mcp_cb_R = ModelCheckpoint(filepath= FILEPATH_R, monitor='val_loss',
                         save_best_only=True, save_weights_only=True, mode='min', verbose=1)
mcp_cb_G = ModelCheckpoint(filepath= FILEPATH_G, monitor='val_loss',
                         save_best_only=True, save_weights_only=True, mode='min', verbose=1)
